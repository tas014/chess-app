...
const Notation = ({ mobileOpen }) => {
  ...
  const notation = (moves, isCheck, isCheckmate) => {
    ...
    if (to.event === "castlesShort")
      return `O-O${isCheckmate ? "#" : ""}${
        isCheck && !isCheckmate ? "+" : ""
      }`;
    if (to.event === "castlesLong")
      return `O-O-O${isCheckmate ? "#" : ""}${
        isCheck && !isCheckmate ? "+" : ""
      }`;
    if (to.event === "promotion")
      return `${
        toPiece ? fromPiece[0] + "x" + landingSquare : landingSquare
      }=${getPieceNotation(to, to.queeningValue)}${isCheckmate ? "#" : ""}${
        isCheck && !isCheckmate ? "+" : ""
      }`;
    if (to.event === "enPassant")
      return `${fromPiece[0]}x${translateCoordToNotation(from.x, to.y)}${
        isCheckmate ? "#" : ""
      }${isCheck && !isCheckmate ? "+" : ""}`;
    return `${fromPiece[0]}${
      isPawn && !toPiece
        ...
        ? landingSquare
        : "x" + landingSquare
    }${isCheckmate ? "#" : ""}${isCheck && !isCheckmate ? "+" : ""}`;
  };

  return (
    <div
      className={`${styles["moves-list-container"]} ${
    ...
    >
      ...
      <ul className={styles["moves-list"]}>
        {movesList.length > 0 ? (
          movesList.map(({ moveData, isCheck, isCheckmate }, ind) => (
            <li key={ind}>{notation(moveData, isCheck, isCheckmate)}</li>
          ))
        ) : (
