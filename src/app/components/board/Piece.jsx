...
const Piece = ({ Content, color, position, wasTaken, boardSize, animationsOn }) => {
    const { x, y } = position;
    ...
    const animVerticalMidpoint = !color ? piecePositionValues.top-150 : piecePositionValues.top + 30;
    const animHorizontalMidpoint = animDirection ? piecePositionValues.left + 100 : piecePositionValues.left-100;
    const animHorizontalEndpoint = animDirection ? animHorizontalMidpoint + 200 : animHorizontalMidpoint - 200;   
    ...
    return (
        <div style={{...inlineStyles, ...takenAnimationStyles}} className={`${styles.pieceIcon} ${animationsOn ? styles.transition : ''} ${defineClassName()}`}>
            <Content  
